"""CAPA workflows against extracted PE artifacts."""
...
from memscope_engine.providers.capa import CapaProvider, bundled_capa_roots
...
def get_or_create_provider(paths: AppPaths, db: Database) -> CapaProvider:
    provider = CapaProvider(
        tools_dir=paths.tools,
        analysis_dir=paths.analysis,
        artifacts_dir=paths.artifacts,
        extra_tool_roots=bundled_capa_roots(),
    )
    row = db.fetchone("SELECT value_json FROM app_settings WHERE key = 'capa'")
    if row:
        try:
...
def save_capa_settings(db: Database, provider: CapaProvider) -> dict[str, Any]:
    payload = {
    ...
    db.execute(
        """
        INSERT INTO app_settings(key, value_json, updated_at) VALUES('capa', ?, ?)
...
def capa_status(paths: AppPaths, db: Database) -> dict[str, Any]:
    return get_or_create_provider(paths, db).availability()


def configure_capa(paths: AppPaths, db: Database, settings: dict[str, Any]) -> dict[str, Any]:
    provider = get_or_create_provider(paths, db)
    provider.configure(settings)
    return save_capa_settings(db, provider)


def run_capa_artifact_job(
    db: Database,
    params: dict[str, Any],
    ...
    *,
    ...
    if not artifact_id:
        raise AppError(code="artifact_required", message="artifact_id is required for CAPA.", entity="capa")
                ...
                [
                    {
                        ...
                        "reason": "Static capability analysis of an extracted PE artifact",
                    }
                ]
            ),
        ),
    ...
    db.execute(
        """
        INSERT INTO capa_scans (
          id, evidence_id, artifact_id, process_id, pid, memory_region_id,
          pe_extraction_run_id, analysis_run_id, job_id, status, capa_version,
          executable_path, capability_count, exit_code, output_json_path,
    ...
    try:
        ...
        for cap in result.get("capabilities") or []:
            db.execute(
                ...
                (
                    ...
                    cap.get("namespace"),
                    cap.get("scope"),
                    json.dumps(cap.get("attck") or []),
                    json.dumps(cap.get("mbc") or []),
                    json.dumps(cap.get("authors") or []),
                    cap.get("description"),
                    _utcnow(),
                ),
            )
        db.execute(
            """
            UPDATE capa_scans SET status='completed', capa_version=?, executable_path=?,
              capability_count=?, exit_code=?, output_json_path=?, observed_json=?,
              interpretation_json=?, finished_at=? WHERE id=?
            """,
            (
                result.get("capa_version"),
                result.get("executable_path"),
                result.get("capability_count") or 0,
        ...
        db.execute(
            ...
            (
                ...
                json.dumps(
                    {
                        "tool": "capa",
                        "tool_version": result.get("capa_version"),
                        "artifact_id": artifact_id,
        ...
        db.execute(
            ...
            ) VALUES (?, ?, NULL, 'analysis_time', 'inferred', 'capa_scan', ?, ?, ?,
              'artifact', ?, 'capa_scans', 'provider.capa', ?, ?)
            """,
            (
                str(uuid4()),
                evidence_id,
                f"CAPA on {art['filename']}: {result.get('capability_count') or 0} capability(ies)",
                art.get("process_id"),
                art.get("pid"),
                artifact_id,
                json.dumps(
                    {
                        ...
                        "capability_count": result.get("capability_count") or 0,
                    }
                ),
                _utcnow(),
            ),
        )
        return get_capa_scan(db, scan_id)
...
def _fail(db: Database, exec_id: str, run_id: str, scan_id: str, exc: AppError) -> None:
    ...
    db.execute(
        "UPDATE capa_scans SET status=?, error_json=?, finished_at=? WHERE id=?",
...
def get_capa_scan(db: Database, scan_id: str) -> dict[str, Any]:
    row = db.fetchone("SELECT * FROM capa_scans WHERE id = ?", (scan_id,))
    if not row:
        raise AppError(code="capa_scan_missing", message="CAPA scan not found.", entity="capa")
    caps = db.fetchall(
        "SELECT * FROM capa_capabilities WHERE scan_id = ? ORDER BY namespace, name",
        (scan_id,),
    )
    return {"scan": _scan_dto(row), "capabilities": [_cap_dto(c) for c in caps]}


def list_capa_scans_for_artifact(db: Database, artifact_id: str) -> dict[str, Any]:
    rows = db.fetchall(
        "SELECT * FROM capa_scans WHERE artifact_id = ? ORDER BY started_at DESC LIMIT 50",
        (artifact_id,),
    )
    items = []
    for r in rows:
        ...
        items.append({"scan": _scan_dto(r), "capabilities": [_cap_dto(c) for c in caps]})
...
def _scan_dto(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "status": row["status"],
        "capa_version": row.get("capa_version"),
...
def _cap_dto(row: dict[str, Any]) -> dict[str, Any]:
    def _j(key: str, default: Any) -> Any:
        try:
            return json.loads(row.get(key) or json.dumps(default))
        except json.JSONDecodeError:
