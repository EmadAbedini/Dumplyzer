...
def collect_executed(
    ...
    """Return (executed ids, failed ids, ids a running job has not finished yet)."""
...
def coverage_for_evidence(db: Database, evidence_id: str) -> dict[str, Any]:
    executed, failed, pending = collect_executed(db, evidence_id)
    ...
    for cid in ALL_CAPABILITY_IDS:
        ...
        if cid == "command_lines":
            ...
            n = int(count or 0)
            if cid in pending:
                items[cid] = {"id": cid, "state": "not_analyzed", "count": n}
            elif n > 0 and (cid in executed or in_progress):
                items[cid] = {
                    "id": cid,
                    "state": "analyzed" if cid in executed else "not_analyzed",
                    "count": n,
                }
            elif cid in failed:
                items[cid] = {"id": cid, "state": "failed", "count": None}
            else:
                items[cid] = {"id": cid, "state": "not_analyzed", "count": None}
            continue
        if cid in executed:
            n = int(count or 0)
            if cid in ("recommended", "process_deep_dive"):
                ...
                state = "analyzed_zero"
            items[cid] = {"id": cid, "state": state, "count": n}
        elif cid in failed:
            items[cid] = {"id": cid, "state": "failed", "count": None}
        else:
            n = int(count or 0)
            show_count = cid in pending or (n > 0 and in_progress)
            items[cid] = {
                "id": cid,
                "state": "not_analyzed",
                "count": n if show_count else None,
            }
