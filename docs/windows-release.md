...
This procedure produces installers. It does **not** perform a clean-machine installation test.
...
- Clean-machine install must be run on a VM/host without developer Python/Node/Rust. See `docs/clean-machine-validation.md`.
- Per-user install directories are user-writable (Windows per-user reality).
- WebView2 Evergreen bootstrapper is embedded; a fully offline WebView2 absent machine may still need the runtime.
- Volatility plugins that need the `full` extra (capstone, pycryptodome, yara-python) may appear as import failures. That is intentional: YARA stays optional and those extras are not bundled.
- Code signing / Authenticode is not configured.
