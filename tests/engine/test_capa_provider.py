"""CAPA provider, command construction, parsing, and provenance tests."""

from __future__ import annotations

import sys
from pathlib import Path
from uuid import uuid4

import pytest

from memscope_engine.analysis import capa_workflows
from memscope_engine.analysis.workflows import import_evidence
from memscope_engine.artifacts import store as artifact_store
from memscope_engine.errors import AppError
from memscope_engine.paths import AppPaths
from memscope_engine.providers.capa import (
    CapaProvider,
    build_scan_argv,
    build_version_argv,
    normalize_capa_json,
    parse_version_output,
)
from memscope_engine.providers.process_run import ProcessRun
from memscope_engine.server import HANDLERS, handle_app_init
from memscope_engine.storage import Database
from memscope_engine.storage.schema import SCHEMA_VERSION


def _write_dummy_exe(path: Path) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(b"MZ" + b"\x00" * 256)
    return path


def test_schema_capa(tmp_path: Path) -> None:
    db = Database(tmp_path / "t.db")
    assert db.schema_version() == SCHEMA_VERSION
    db.execute("SELECT COUNT(*) AS c FROM capa_scans")
    db.execute("SELECT COUNT(*) AS c FROM capa_capabilities")
    db.close()


def test_availability_missing(tmp_path: Path) -> None:
    paths = AppPaths(tmp_path / "data").ensure()
    p = CapaProvider(tools_dir=paths.tools, artifacts_dir=paths.artifacts)
    avail = p.availability()
    assert avail["available"] is False
    assert avail["license"]["bundled_in_memscope"] is True


def test_version_and_argv(tmp_path: Path) -> None:
    assert parse_version_output("capa 9.4.0") == "9.4.0"
    exe = _write_dummy_exe(tmp_path / "capa.exe")
    sample = tmp_path / "sample.exe"
    sample.write_bytes(b"MZ" + b"\x00" * 64)
    argv = build_scan_argv(exe, sample)
    assert argv[0] == str(exe)
    assert "-j" in argv
    assert str(sample) in argv
    assert "extra" not in argv
    assert build_version_argv(exe) == [str(exe), "--version"]


def test_missing_executable_analyze(tmp_path: Path) -> None:
    p = CapaProvider(tools_dir=tmp_path / "tools")
    sample = tmp_path / "s.exe"
    sample.write_bytes(b"MZ" + b"\x00" * 32)
    with pytest.raises(AppError) as ei:
        p.analyze_pe(sample)
    assert ei.value.code == "capa_unavailable"


def test_parse_capabilities() -> None:
    doc = {
        "meta": {"version": "9.4.0", "sample": {"sha256": "abc"}, "analysis": {"arch": "i386"}},
        "rules": {
            "allocate RWX memory": {
                "meta": {
                    "name": "allocate RWX memory",
                    "namespace": "host-interaction/process/inject",
                    "att&ck": ["Defense Evasion::Process Injection [T1055]"],
                    "mbc": [],
                }
            },
            "http communication": {
                "meta": {"name": "http communication", "namespace": "communication/http"}
            },
        },
    }
    out = normalize_capa_json(doc)
    assert out["capability_count"] == 2
    assert out["interpretation"]["kind"] == "capability"
    assert "malware verdict" in out["interpretation"]["summary"]


def test_analyze_known_pe_with_mock_runner(tmp_path: Path) -> None:
    paths = AppPaths(tmp_path / "data").ensure()
    exe = _write_dummy_exe(paths.tools / "capa" / "capa.exe")
    sample = tmp_path / "sample.exe"
    sample.write_bytes(b"MZ" + b"\x00" * 80)
    payload = {
        "meta": {"version": "9.4.0"},
        "rules": {"create process": {"meta": {"name": "create process", "namespace": "host-interaction/process/create"}}},
    }

    def runner(argv, **_k):
        if "--version" in argv:
            return ProcessRun(returncode=0, stdout="capa 9.4.0\n", stderr="")
        return ProcessRun(returncode=0, stdout=__import__("json").dumps(payload), stderr="")

    p = CapaProvider(tools_dir=paths.tools, artifacts_dir=paths.artifacts, runner=runner)
    result = p.analyze_pe(sample)
    assert result["capability_count"] == 1
    assert result["capabilities"][0]["name"] == "create process"


def test_failure_invalid_json(tmp_path: Path) -> None:
    paths = AppPaths(tmp_path / "data").ensure()
    _write_dummy_exe(paths.tools / "capa.exe")
    sample = tmp_path / "sample.exe"
    sample.write_bytes(b"MZ" + b"\x00" * 80)

    def runner(argv, **_k):
        if "--version" in argv:
            return ProcessRun(returncode=0, stdout="capa 9.4.0\n", stderr="")
        return ProcessRun(returncode=1, stdout="not-json", stderr="err")

    p = CapaProvider(tools_dir=paths.tools, artifacts_dir=paths.artifacts, runner=runner)
    with pytest.raises(AppError) as ei:
        p.analyze_pe(sample)
    assert ei.value.code == "capa_exec_failed"
    assert "non-zero exit code" in ei.value.message


def test_timeout_returns_structured_error(tmp_path: Path) -> None:
    paths = AppPaths(tmp_path / "data").ensure()
    _write_dummy_exe(paths.tools / "capa.exe")
    sample = tmp_path / "sample.exe"
    sample.write_bytes(b"MZ" + b"\x00" * 80)

    def runner(argv, **_k):
        if "--version" in argv:
            return ProcessRun(returncode=0, stdout="capa 9.4.0\n", stderr="")
        return ProcessRun(returncode=-1, stdout="partial", stderr="still running", timed_out=True, tree_terminated=True)

    p = CapaProvider(tools_dir=paths.tools, artifacts_dir=paths.artifacts, runner=runner)
    with pytest.raises(AppError) as ei:
        p.analyze_pe(sample)
    assert ei.value.code == "capa_timeout"
    assert "timed out after 600 seconds" in ei.value.message
    assert "process tree terminated" in ei.value.message
    assert ei.value.data.get("process_tree_terminated") is True
    assert "partial" in (ei.value.details or "")


def test_job_provenance(tmp_path: Path) -> None:
    paths = AppPaths(tmp_path / "data").ensure()
    db = Database(paths.db_path)
    img = tmp_path / "img.raw"
    img.write_bytes(b"dump")
    ev = import_evidence(db, str(img))
    adir = artifact_store.artifact_dir(paths, ev["id"])
    apath = adir / "pid100_process_image_app_400000.exe"
    apath.write_bytes(b"MZ" + b"\x00" * 80)
    art_id = str(uuid4())
    db.execute(
        """
        INSERT INTO artifacts (
          id, evidence_id, process_id, pid, memory_region_id, filename, stored_path,
          sha256, size_bytes, file_type, extraction_method, source_plugin, tool_name,
          tool_version, start_vpn, end_vpn, extracted_at, notes, metadata_json
        ) VALUES (?, ?, NULL, 100, NULL, ?, ?, ?, ?, 'pe', 'volatility3.windows.pedump.process_image',
          'windows.pedump', 'volatility3', '2.28.0', '0x400000', NULL, '2020-01-01T00:00:00+00:00',
          'Extracted PE artifact', ?)
        """,
        (
            art_id,
            ev["id"],
            apath.name,
            str(apath),
            artifact_store.sha256_file(apath),
            apath.stat().st_size,
            '{"pe_extraction_run_id": "run-1", "label": "extracted_pe_artifact"}',
        ),
    )
    _write_dummy_exe(paths.tools / "capa.exe")
    payload = {
        "meta": {"version": "9.4.0"},
        "rules": {"persist via registry": {"meta": {"name": "persist via registry", "namespace": "persistence/registry"}}},
    }

    def runner(argv, **_k):
        if "--version" in argv:
            return ProcessRun(returncode=0, stdout="capa 9.4.0\n", stderr="")
        return ProcessRun(returncode=0, stdout=__import__("json").dumps(payload), stderr="")

    orig = capa_workflows.get_or_create_provider

    def patched(paths_, db_):
        p = orig(paths_, db_)
        p.runner = runner
        return p

    capa_workflows.get_or_create_provider = patched  # type: ignore[method-assign]
    try:
        result = capa_workflows.run_capa_artifact_job(
            db,
            {"artifact_id": art_id, "evidence_id": ev["id"]},
            lambda: False,
            lambda _m: None,
            paths=paths,
        )
        assert result["scan"]["pe_extraction_run_id"] == "run-1"
        assert result["scan"]["status"] == "completed"
        assert result["capabilities"][0]["namespace"] == "persistence/registry"
        scans = capa_workflows.list_capa_scans_for_artifact(db, art_id)
        assert scans["total"] == 1
    finally:
        capa_workflows.get_or_create_provider = orig  # type: ignore[method-assign]
    db.close()


def test_ipc_status(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("DUMPLYZER_BUNDLE_TOOLS", raising=False)
    isolated = tmp_path / "isolated-python.exe"
    isolated.write_bytes(b"")
    monkeypatch.setattr(sys, "executable", str(isolated))
    handle_app_init({"data_dir": str(tmp_path / "ipc")})
    status = HANDLERS["capa.status"]({})
    assert status["available"] is False
    assert "capa.scan_artifact" in HANDLERS
